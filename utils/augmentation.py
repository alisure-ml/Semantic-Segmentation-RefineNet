...
def flip_randomly_left_right_image_with_annotation(image_tensor, annotation_tensor):
    ...
    # Random variable: two possible outcomes (0 or 1) with a 1 in 2 chance
...
def distort_randomly_image_color(image_tensor, fast_mode=False):
    ...
    # uint8转float,且归一化：Make the range to be in [0,1]
...
def scale_randomly_image_with_annotation_with_fixed_size_output(img_tensor, annotation_tensor, output_shape,
    ...
    between values [min_relative_random_scale_change, max_relative_random_scale_change] multiplied
    ...
    rescaling necessary to get image of output_shape, adds random scale jitter, preserves
    scale ratio, masks out unnecessary regions that appear.
    ...
    rand_var = tf.random_uniform(shape=[1], minval=min_relative_random_scale_change,
    ...
    # Resize the image and annotation using nearest neighbour. Be careful -- may cause aliasing.
    resize_img = tf.image.resize_nearest_neighbor(img_batched, scaled_input_shape)
    resize_annotation = tf.image.resize_nearest_neighbor(annotation_batched, scaled_input_shape)

    resize_img = tf.squeeze(resize_img, axis=0)
    resize_annotation = tf.squeeze(resize_annotation, axis=0)
    ...
    annotation_shifted_classes = resize_annotation + 1

    cropped_padded_img = tf.image.resize_image_with_crop_or_pad(resize_img, output_shape[0], output_shape[1])
