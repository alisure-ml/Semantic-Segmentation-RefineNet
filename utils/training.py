...
def get_labels_from_annotation(annotation_tensor, class_labels):
    ...
    # Last value in the classes list should show which number was used in the annotation to mask out
    # the ambigious regions or regions that should not be used for training.
    ...
    # Convert tf.bool to tf.float, Later on in the labels and logits will be used in
    # tf.softmax_cross_entropy_with_logits() function where they have to be of the float type.
    return tf.to_float(labels_2d_stacked)
...
def get_labels_from_annotation_batch(annotation_batch_tensor, class_labels):
    ...
    # 对每一个elem都执行fn():map on the list of tensors unpacked from `elems` on dimension 0.
...
def get_valid_entries_indices_from_annotation_batch(annotation_batch_tensor, class_labels):
    ...
    # Get binary mask for the pixels that we want to use for training. We do this because some pixels
    # are marked as ambigious and we don't want to use them for trainig to avoid confusing the model
    valid_labels_mask = tf.not_equal(annotation_batch_tensor, mask_out_class_label)
    # return (num_true, dim_size(condition))
    valid_labels_indices = tf.where(valid_labels_mask)
    return tf.to_int32(valid_labels_indices)


# 得到需要计算交叉熵的像素点的label和logit
def get_valid_logits_and_labels(annotation_batch_tensor, logits_batch_tensor, class_labels):
    ...
    labels_batch_tensor = get_labels_from_annotation_batch(annotation_batch_tensor, class_labels)
    valid_batch_indices = get_valid_entries_indices_from_annotation_batch(annotation_batch_tensor, class_labels)
